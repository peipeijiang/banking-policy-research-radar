...
[查看原文](http://arxiv.org/abs/2607.19005v1)

## 一句话结论

> 提出可观测矩阵动力学（OMD）方法，通过距离矩阵谱分析S&P 500在三次危机中的相关性结构，发现市场从未学习到稳定几何结构，并识别危机驱动因素。
...
可观测矩阵动力学（Observable Matrix Dynamics, OMD）方法通过固定大小距离矩阵及其谱的轨迹来监测复杂非线性系统的时间演化。我们将其应用于三个危机十年期间的标普500指数（S&P 500）截面数据，即2001年互联网泡沫破裂、2007—2008年金融危机以及2020年新冠疫情崩盘，并在固定标的池中采用三个固定大小的可观测变量。滚动收益率反余弦距离矩阵（arccos distance matrix）解读了相关性几何结构：其有效维度在2008年和2020年危机期间发生坍缩，而2001年泡沫破裂则表现为分散式平仓。与机器学习距离矩阵对照，其谱始终处于未松弛的预学习状态，不存在低维流形，因此市场从未学习其相关性结构或松弛至平稳几何形态。剔除市场因子后，可揭示出连贯的板块轮动，其名称级归因（name-level attribution）能够识别哪些股票驱动了每次危机及其驱动顺序。在短回溯窗口下，这些信号可解析出前兆特征并预测内生的2008年危机，但无法预测外生的2020年冲击。另外两个可观测变量将日收益率和波动率排名建模为其排名空间上的马尔可夫链（Markov chain）。收益率链具有由防御性板块主导的持续领涨股及近似可逆的动力学特征。波动率链的持续性远强于前者，由金融板块主导，且是唯一携带微弱、偶发时间箭头（arrow of time）的变量，该时间箭头在市场压力期间增强，并与波动率聚集效应及Zumbach效应相匹配。所有三个矩阵在市场崩盘期间均呈现出一致的变化特征。
...
_知识库更新时间：2026-07-22T04:03:49.095045_
