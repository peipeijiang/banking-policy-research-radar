---
title: "Asymmetric effects of monetary policy shocks on economic performance: empirical evidence from Turkey"
paper_id: "https://doi.org/10.1080/13504851.2015.1073836"
source: "citation"
published: "2015-08-05T00:00:00"
score: 80.0
tags: ["paper", "banking-fiscal-monetary-policy", "Market Dynamics and Volatility", "Monetary Policy and Economic Impact", "Energy, Environment, Economic Growth"]
---

# Asymmetric effects of monetary policy shocks on economic performance: empirical evidence from Turkey

[查看原文](https://doi.org/10.1080/13504851.2015.1073836)

## 一句话结论

> 该论文利用土耳其1990-2014年月度数据，发现紧缩性货币政策对汇率、产出和价格的影响强于扩张性货币政策，且冲击越大差异越明显。

## 论文信息

- **作者**：Volkan Ülke, Hakan Berument
- **来源**：Applied Economics Letters
- **发布时间**：2015-08-05
- **相关度评分**：80.0
- **DOI**：[https://doi.org/10.1080/13504851.2015.1073836](https://doi.org/10.1080/13504851.2015.1073836)

## 相关性评分

- **商业银行**：0.0/10
- **货币政策**：8.0/10（最高匹配）
- **财政政策**：0.0/10
- **评分依据**：论文摘要

> 最高匹配领域：货币政策 8.0/10。论文研究货币政策冲击对汇率、产出和通胀的非对称效应，使用非线性VAR模型，核心是货币政策工具（利率）的传导机制和效果，属于货币政策领域核心研究。未涉及商业银行或财政政策内容。

<details open>
<summary><strong>中文摘要</strong></summary>

本研究利用1990年至2014年的月度数据，探讨货币政策冲击对新兴经济体——土耳其——的汇率、产出和通胀等宏观经济变量的非对称效应。我们采用Kilian和Vigfusson（2011）提出的创新性非线性向量自回归模型（nonlinear vector autoregressive model, NVAR），该模型使我们能够观察不同立场（紧缩或宽松）及不同规模（小或大）的货币政策行动所产生的影响。实证结果表明，紧缩性货币政策（此处表现为利率的正向冲击）会如经济理论所预期的那样，降低汇率、产出和价格水平。宽松性货币政策（此处表现为利率的负向冲击）则对这些变量产生相反的影响。然而，宽松性货币政策的效果弱于紧缩性货币政策，因为宽松性货币政策冲击的有效性低于紧缩性货币政策冲击。此外，随着冲击幅度的增大，紧缩性与宽松性货币政策效果之间的差异也会扩大。

</details>

<details>
<summary><strong>英文摘要</strong></summary>

This study investigates the asymmetric effects of monetary policy shocks on the macroeconomic variables of exchange rate, output and inflation for an emerging economy ‒ Turkey ‒ by using monthly data between 1990 and 2014. We employ the innovative nonlinear vector autoregressive model of Kilian and Vigfusson (2011), which allows us to observe the effect of different stances (tight or loose) and different sizes (small or large) of monetary policy actions. Our empirical evidence reveals that tight monetary policy, which, in this case, is captured with a positive shock to interest rate, decreases exchange rate, output and prices, as economic theory suggests. Loose monetary policy, which is captured with a negative shock to interest rate, has the opposite effect on these variables. However, the effects of loose monetary policy are weaker than the effects of tight monetary policy because loose monetary policy shocks are less effective than tight monetary policy shocks. Moreover, as the magnitude of a shock increases, the difference between the effects of tight and loose monetary policy policies also increases.

</details>

<details>
<summary><strong>发现与关联证据</strong></summary>

- **channel**：citation_expansion
- **relation**：related_to_seed
- **seed_paper_id**：https://doi.org/10.2139/ssrn.965470
- **seed_title**：Bank Capital Channels in the Monetary Transmission Mechanism
- **seed_score**：121.0

</details>

---

_知识库更新时间：2026-07-13T06:39:35.599633_
