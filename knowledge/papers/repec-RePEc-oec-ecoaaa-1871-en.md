---
title: "Assessing risks to fiscal policy: Calibrating stochastic debt sustainability analysis models"
paper_id: "repec:RePEc:oec:ecoaaa:1871-en"
source: "oecd"
published: "2026-07-13T00:00:00"
score: 80.0
tags: ["paper", "banking-fiscal-monetary-policy"]
---

# Assessing risks to fiscal policy: Calibrating stochastic debt sustainability analysis models

[查看原文](https://ideas.repec.org/p/oec/ecoaaa/1871-en.html)

## 一句话结论

> 该论文通过校准随机债务可持续性分析模型，提出了一种状态依存的评估方法，发现经济繁荣时期财政风险高于传统方法，需要更大的结构性初级余额来确保财政缓冲。

## 论文信息

- **作者**：Sebastian Barnes, Eddie Casey, Fred Hanmer
- **来源**：OECD Economics Department Working Papers
- **发布时间**：2026-07-13
- **相关度评分**：80.0
- **DOI**：10.1787/546c0ea7-en

## 相关性评分

- **商业银行**：0.0/10
- **货币政策**：0.0/10
- **财政政策**：8.0/10（最高匹配）
- **评分依据**：论文摘要

> 最高匹配领域：财政政策 8.0/10。论文聚焦于随机债务可持续性分析（SDSA）模型的校准方法，用于评估财政政策风险。研究问题、方法和结果直接属于财政政策领域，涉及公共债务、财政缓冲和结构性初级余额调整。未涉及商业银行或货币政策内容。

<details open>
<summary><strong>中文摘要</strong></summary>

随机债务可持续性分析（Stochastic Debt Sustainability Analysis, SDSA）是评估现代财政政策制定中风险的关键工具。然而，设计并校准这些模型面临重大挑战。本文利用涵盖1980年以来的最多24个OECD国家面板数据，论证了跨国数据整合能更典型地反映财政风险，有效延长样本周期，且趋势-周期方法能更好地解释结构性变化。校准方法的选择具有政策相关性，因其会显著改变模型所隐含的、为稳定债务比率所需的基本财政收支调整规模。鉴于经济上行期间实时宏观经济风险倾向于下行，本文提出了一种基于当期产出缺口估计的状态依存随机债务可持续性分析（State-Contingent SDSA, SC-SDSA）方法。这表明经济繁荣时期的风险高于传统方法所揭示的水平，且需要比正常时期更大的结构性基本财政收支盈余，以确保充足的财政缓冲。

</details>

<details>
<summary><strong>英文摘要</strong></summary>

Stochastic debt sustainability analysis (SDSA) is a key tool for assessing risks in modern fiscal policymaking. Yet, designing and calibrating these models poses significant challenges. Using a panel of up to 24 OECD countries with data spanning from 1980, this paper argues that pooling data across countries provides a more representative reflection of fiscal risks, effectively extends sample periods, and a trend-cycle approach better accounts for structural changes. The choice of calibration method is policy relevant as it substantially alters the required primary balance adjustments to stabilise the debt ratio implied by the models. Given that real-time macroeconomic risks are skewed to the downside during economic upswings, the paper proposes a state-contingent (SC-SDSA) approach conditioned on contemporaneous output gap estimates. This suggests that risks are greater during economic booms than conventional approaches and that larger structural primary balances are required than in normal times to ensure adequate fiscal buffers.

</details>

<details>
<summary><strong>发现与关联证据</strong></summary>

- **provider**：IDEAS/RePEc
- **series_url**：https://ideas.repec.org/s/oec/ecoaaa.html
- **free_download**：True
- **date_precision**：day

</details>

---

_知识库更新时间：2026-07-13T04:30:56.246410_
