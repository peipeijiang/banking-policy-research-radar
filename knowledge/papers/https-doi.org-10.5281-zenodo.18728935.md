...
paper_id: "https://doi.org/10.5281/zenodo.18728935"
source: "openalex"
published: "2026-07-12T00:00:00"
score: 0.0
tags: ["paper", "banking-fiscal-monetary-policy", "Cosmology and Gravitation Theories", "Galaxies: Formation, Evolution, Phenomena", "Computational Physics and Python Applications"]
---

# The Curvature Relaxation Model: A Four-Paper Program for Geometric Cosmology Without the Dark Sector

[查看原文](https://doi.org/10.5281/zenodo.18728935)

## 一句话结论

> 该论文提出曲率松弛模型，通过几何框架从标量-曲率动力学推导暗物质现象，无需假设新粒子，并预测了可检验的宇宙学信号。
...
- **发布时间**：2026-07-12
- **相关度评分**：0.0
- **DOI**：[https://doi.org/10.5281/zenodo.18728935](https://doi.org/10.5281/zenodo.18728935)

<details open>
<summary><strong>中文摘要</strong></summary>

以下是一组四篇系列论文，提出曲率弛豫模型（Curvature Relaxation Model, CRM），这是一个无需假设新粒子、仅从标量-曲率动力学推导暗物质现象的几何框架。核心论文 论文一：博弈论基础——通过热力学博弈论（雅各布森传统）建立CRM，从膨胀与引力之间的纳什均衡推导出曲率饱和。论文二：MOND的涌现——表明修正牛顿动力学（Modified Newtonian Dynamics, MOND）作为有效背景耦合从运行曲率参数β ≈ 2.0中涌现，该参数通过Pantheon+和宇宙微波背景（CMB）数据拟合得到。论文三：拉格朗日形式——提供包含Pöschl–Teller标量势的协变拉格朗日量、经马尔可夫链蒙特卡洛（MCMC）拟合的f(R)参数、CMB Cℓ谱以及S₈预测。论文四：矢量扇区与星系动力学——通过一个有质量的Proca型矢量场（子代2）扩展标量扇区，推导出a₀ ≈ cH₀/(2π)关系，并针对175个SPARC星系进行检验。扩展内容 相关论文五：饱和定理（10.5281/zenodo.19036188）——该伴随论文后续发展，基于CRM框架与泛函稳定性理论的进展，证明论文一至四中的tanh饱和轮廓并非模型选择，而是数学必然性：任何满足四个最小公理条件的量子引力理论都必须产生tanh形式。所有主要量子引力（QG）方案（圈量子引力、渐近安全、弦论、因果集、非交换几何）均被证明满足这些公理。相关论文六：QG-CRM：紫外完备化（10.5281/zenodo.19352448，草稿）——该伴随论文回答了论文五中的开放问题：“哪个紫外完备化选择了k和Φ₀？”通过将CRM拉格朗日量中的γR²扇区识别为渐近自由量子二次引力（Quantum Quadratic Gravity, QQG），暴胀通过重整化群（RG）运行动态产生，无需暴胀子场。饱和定理提供了唯一的紫外-红外界面。预测：nₛ ∼ 1 − 4/(3N) ≈ 0.976，r ≥ 0.01，可通过第四代CMB实验检验。更新日志：论文一至四 版本7.0变更（2026-03-26）：文献验证与更正：FLAMINGO作者更正（Schaye → McCarthy，论文三），Chou年份更正（2017 → 2016，论文三DE），Prigogine参考文献更正（Nicolis & Prigogine，论文四），Cardoso → Nakamoto & Oshita（论文三DE）。修复：所有论文中单位统一为“Bernau, Germany”，删除复制粘贴重复内容（论文二EN），清理英文论文中的德文章节注释（论文二+三）。标题更正：“暗物质”改为“暗扇区”（更准确的范围）。版本8.0变更（2026年7月）：本版本新增对路线三后继架构线性扇区的首次符号分析，完成论文三和四的修补包，并实现整个系列德文/英文结构的同步。论文四——新增章节“路线三作用量的线性扇区：两门一通道”：推导了FLRW背景代数（守恒平移荷、能量密度与压强、代数拉格朗日乘子）；有质量矢量-标量模式精确再现已发表的AeST闵可夫斯基色散关系；正则（Dirac/Faddeev–Jackiw）约化表明，对于k < k_star（其中k_star² = K_QQ Qbar² G / (beta F_Y)），二次哈密顿量无下界——因此路线三继承了AeST的低k哈密顿量问题，现记录为明确的定量门限；张量扇区给出c_T = 1，α_T = 0恒成立，因此GW170817由构造自动满足。准静态响应仅作为校准后的局域诊断报告（在相同k_star处存在候选极点），而非确定结果。论文四——撤回：先前考虑的准静态滑移公式及参数约束F_Y >> beta被撤回；不声称候选响应在极点处或进入哈密顿量无界分支的延续。论文三：f(R)归一化约定（ε = 16 π G γ）现已在两种语言版本中明确陈述，所有质量/康普顿/屏蔽公式均以此表示；hi_class透明度说明更新至验证后状态（代理分支通过稳定性测试；大标量声速仍为开放解释门限）。论文一至二：来源维护与引文检查；无主张变更。德/英同步：论文一至四的德文版本现与英文版本具有相同结构（相同标签集）；此前阻碍上传的论文二DE缺口已填补。状态：该模型仍为候选方案。透镜、参数化后牛顿形式（PPN）、CMB、结构形成以及统一的SPARC拟合仍为开放问题；a₀ = c H₀/(2π)仍为校准锚点，而非推导结果。版本6.0变更（2026-03-13）：回溯影响修订（所有论文共11个REV点），文献更新（8个LIT-CRM点，包括Brodie 2026、DES Y6、Famaey & McGaugh 2012、模仿引力、Jacobson非黎曼2026），孤子系综紫外完备化候选方案（论文三），完整英/德同步，重复bibitem修复（论文一）。
