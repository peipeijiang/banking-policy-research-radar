...
[查看原文](http://arxiv.org/abs/2607.29442v2)

## 一句话结论

> 该论文提出一个微观结构模型，将逐笔价格变动分解为永久性变动和短暂噪声，并证明该模型在宏观尺度上收敛于粗糙噪声过程，同时开发了GMM估计方法和检验，应用于道琼斯工业平均指数成分股数据。

## 论文信息

- **作者**：Peter Korsbakke Christensen, Anders Norlyk
- **来源**：arxiv
- **发布时间**：2026-07-31
- **相关度评分**：0.0
- **DOI**：-
...
近期有研究提出，用具有连续样本路径且比标准布朗运动更粗糙的连续时间过程来刻画价格中的微观结构噪声。本文提出一个针对逐笔价格变动的微观结构模型，该模型明确区分了永久性价格变动与由噪声引起的短暂价格变动。我们展示了该模型如何收敛到一个标准的半鞅模型，用于描述永久价格过程，同时包含一个源自宏观尺度上短暂价格变动的粗糙噪声项。这为粗糙噪声模型提供了微观结构基础。随后，我们开发了一种适用于逐笔数据的广义矩估计（GMM）方法，并配套提出了一个正式的粗糙噪声检验。通过模拟研究，我们证明了该估计量和检验在有限样本中的有效性，并将其应用于2024年道琼斯工业平均指数成分股的逐笔数据。由于我们的估计量专为逐笔数据设计，我们在日度层面估计粗糙度，揭示了显著的日间波动。我们发现，粗糙噪声虽然存在，但并非普遍现象：即使被检测到，粗糙度指数通常也接近零，并且在以短期价格反转为主导的交易日中最为显著。
...
_知识库更新时间：2026-08-05T03:52:51.095128_
