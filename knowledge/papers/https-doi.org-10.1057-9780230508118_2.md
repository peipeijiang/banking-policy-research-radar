---
title: "Liquidity and Financial Operations"
paper_id: "https://doi.org/10.1057/9780230508118_2"
source: "citation"
published: "2005-01-01T00:00:00"
score: 0.0
tags: ["paper", "banking-fiscal-monetary-policy", "Banking stability, regulation, efficiency"]
---

# Liquidity and Financial Operations

[查看原文](https://doi.org/10.1057/9780230508118_2)

## 一句话结论

> 该论文介绍了流动性在金融操作中的重要性，包括流动性需求、风险收益权衡以及不同行业的流动性特征，但未涉及具体的银行信贷、货币政策或财政政策等主题。

## 论文信息

- **作者**：Erik Banks
- **来源**：Palgrave Macmillan UK eBooks
- **发布时间**：2005-01-01
- **相关度评分**：0.0
- **DOI**：[https://doi.org/10.1057/9780230508118_2](https://doi.org/10.1057/9780230508118_2)

<details open>
<summary><strong>中文摘要</strong></summary>

在了解流动性及其相关风险的基本背景后，我们现在准备审视流动性在金融运作中的作用。通过考察与运营环境相关的概念，我们从内部和系统性的角度理解流动性的重要性，这将在本书第二部分探讨流动性引发的问题时有所助益。本章将探讨流动性运营要求、流动性风险与收益的权衡、不同行业的流动性特征，以及内生流动性（endogenous liquidity）与外生流动性（exogenous liquidity）。  
关键词：现金流（Cash Flow）、金融机构（Financial Institution）、共同基金（Mutual Fund）、对冲基金（Hedge Fund）、财务困境（Financial Distress）  
以上关键词由机器添加，并非作者提供。此过程为实验性，关键词可能随学习算法的改进而更新。

</details>

<details>
<summary><strong>英文摘要</strong></summary>

With some background on liquidity and its associated risks, we are now prepared to review the role of liquidity in financial operations. By examining concepts related to the operating environment we come to understand the importance of liquidity from an internal and systemic perspective, which will be useful when we consider liquidity-induced problems in Part II of the book. In this chapter we explore liquidity operating requirements, the liquidity risk/return trade-off, liquidity characteristics across industries, and endogenous versus exogenous liquidity.KeywordsCash FlowFinancial InstitutionMutual FundHedge FundFinancial DistressThese keywords were added by machine and not by the authors. This process is experimental and the keywords may be updated as the learning algorithm improves.

</details>

<details>
<summary><strong>发现与关联证据</strong></summary>

- **channel**：citation_expansion
- **relation**：related_to_seed
- **seed_paper_id**：https://doi.org/10.2139/ssrn.1951799
- **seed_title**：A Pigovian Approach to Liquidity Regulation
- **seed_score**：111.0

</details>

---

_知识库更新时间：2026-07-14T03:57:10.401065_
