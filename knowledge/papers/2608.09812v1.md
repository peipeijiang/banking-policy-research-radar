...
[查看原文](http://arxiv.org/abs/2608.09812v1)

## 一句话结论

> 研究有界潜在结果下随机实验的最优设计与估计，提出独立随机分配和特殊回归方法以最小化最坏情况均方误差。
...
我研究了在已知潜在结果有界（如二元结果）的情况下，用于估计有限总体平均处理效应的随机化实验的最优设计与分析。在所有分配机制和一类广泛的仿射估计量中，最坏情况下的均方误差（MSE）通过独立随机分配以及对以支持域中点为中心的结果变量关于重新中心化的处理变量进行无截距的非传统回归而达到最小。这与通常采用的平衡完全随机化和均值差估计的建议形成对比：当结果有界时，实际处理份额中的随机性具有信息量。相对于全样本完全随机化，最坏情况下的增益在渐近意义上是小的，但相对于其他设计，增益可能是一阶的：完全配对随机化和配对固定效应回归的最坏情况MSE是其两倍。我将结果扩展到允许任意估计量。独立随机分配仍然是最优的，而一般非线性的最优估计量能够显著降低最坏情况下的MSE。
...
_知识库更新时间：2026-08-12T03:15:19.477049_
