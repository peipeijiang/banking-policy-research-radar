...
[查看原文](http://arxiv.org/abs/2608.09069v2)

## 一句话结论

> 论文提出针对持续自适应的生成式AI模型的遥测架构和对抗性隐藏对策，以解决模型风险管理中的治理问题。
...
模型风险管理（MRM）指引假设了一个静态的模型生命周期，在此生命周期中，模型被开发、独立验证并实施，而无需进一步的自主修改。持续自适应的生成式人工智能系统——即在生产部署期间自行更新权重的模型——从根本上违反了这一假设，使得时点验证不再充分。本文分两部分解决由此产生的治理问题。第一部分为这类模型构建了一套严格的遥测架构，使其同时在离散时间和连续时间下运行。我们确立了用于审计目的的最小充分统计量，构建了针对离散权重序列的防篡改默克尔链，通过伊藤公式推导了相应的连续时间推广，并提出了基于KL散度停时的事件驱动日志记录方法，该方法在计算上可行且对验证具有实际意义。第二部分探讨当模型提供方具有对抗性时会发生什么。部署此类模型的企业有强烈动机隐瞒会触发强制验证审查的学习更新。我们将此形式化为“模型隐藏问题”，并针对第一部分架构提供了一套系统的分类体系，涵盖六种不同的攻击策略，横跨离散与连续时间，并为每种策略给出了正式的对策。两部分共同建立了一种双机制架构，其中连续遥测是必要但不充分的，它缩小了——但从未取代——定期侵入式审计的范围。该框架与模型架构无关，旨在满足传统MRM的三大支柱要求。
...
_知识库更新时间：2026-08-12T03:15:19.481039_
