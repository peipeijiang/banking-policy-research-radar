---
title: "Ambiguity and the Value of Late Resolution of Uncertainty"
paper_id: "repec:RePEc:ces:ceswps:_12804"
source: "cesifo"
published: "2026-01-01T00:00:00"
score: 0.0
tags: ["paper", "banking-fiscal-monetary-policy"]
---

# Ambiguity and the Value of Late Resolution of Uncertainty

[查看原文](https://ideas.repec.org/p/ces/ceswps/_12804.html)

## 一句话结论

> 该论文证明在动态多源不确定性环境中，递归模糊厌恶偏好可能导致对不确定性晚解决的偏好，并通过简单瓮问题说明机制。

## 论文信息

- **作者**：Asen Kochov
- **来源**：CESifo Working Paper Series
- **发布时间**：2026-01-01
- **相关度评分**：0.0
- **DOI**：-

<details open>
<summary><strong>中文摘要</strong></summary>

本文表明，在具有多重不确定性来源的动态设定中，递归模糊厌恶偏好（recursive ambiguity-averse preferences）可能隐含对不确定性晚揭示的偏好。具体而言，当某一来源较早被揭示且不确定性被递归评估时，两个来源的同时揭示能够提供对冲收益（hedging benefits），而这种收益将被抵消。本文通过一个简单的动态瓮问题（urn problem）阐释了这一机制。在一种特殊情形下，涉及两个瓮的复合赌注（composite bets）的等价静态表述，为直接的实验检验提供了可能。

</details>

<details>
<summary><strong>英文摘要</strong></summary>

The paper shows that in a dynamic setting with multiple sources of uncertainty, recursive ambiguity-averse preferences may imply a preference for late resolution of uncertainty. In particular, the simultaneous resolution of two sources can offer hedging benefits that are negated when one source is resolved earlier and uncertainty is evaluated recursively. The paper illustrates the mechanism in a simple dynamic urn problem. In a special case, an equivalent static formulation involving composite bets on two urns suggests a direct experimental test.

</details>

<details>
<summary><strong>发现与关联证据</strong></summary>

- **provider**：IDEAS/RePEc
- **series_url**：https://ideas.repec.org/s/ces/ceswps.html
- **free_download**：True
- **date_precision**：year

</details>

---

_知识库更新时间：2026-07-14T03:57:10.397982_
