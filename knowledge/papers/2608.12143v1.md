...
[查看原文](http://arxiv.org/abs/2608.12143v1)

## 一句话结论

> 本文通过一个双稳态模型分析气候联盟的稳健性与效率权衡，提出碳货币等架构，并评估其自燃性、创始依赖性和永久支持依赖性。
...
国际气候合作的设计面临着一个权衡：分配效率与制度在遭遇退出、重新谈判和政治更迭侵蚀时的稳健性之间的取舍。我们在一个简化的联盟形成博弈中形式化了这一权衡，其中成员资格由两个基于市场的渠道驱动——成员溢价与外部者流失——并利用稳健控制的方法，在有限制度扰动的范围内实现稳定。每个成员所面临的搭便车缺口源自经济基本要素，即碳价差与出口排放强度，并与制度架构特有的渠道相分离；这种分离使模型可解且具有可比性。该模型是双稳态的：一个小规模残余俱乐部与一个近乎普遍的联盟之间由临界质量分隔。已建立的联盟能够承受的扰动约为点火阶段可容许扰动的十二倍，而这一临界转变在异质性达到约成员溢价七倍的临界无序度之前仍然成立。两个稳健性坐标——点火成本与崩溃阈值——将每种进入架构置于包含三种状态的地图中：自点火型、依赖创始型与依赖持续支持型。碳货币作为一种新提出的基于数量的体系，其排放权每期重新发放、使用后即注销、并在边境强制执行，属于自点火型；边境调整属于依赖创始型；出口退税属于依赖持续支持型。没有外部者流失机制的架构，如清洁证书和排放交易体系链接，构成一个独立类别，它们能在联盟内部提高效率，但无法推动联盟的形成。稳健性决定了合作能否形成并持续；效率则仅决定已建立的联盟能带来多少成果。
...
_知识库更新时间：2026-08-13T03:16:17.610626_
