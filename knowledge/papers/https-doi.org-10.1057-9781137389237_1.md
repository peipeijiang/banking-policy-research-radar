---
title: "Musings on Liquidity"
paper_id: "https://doi.org/10.1057/9781137389237_1"
source: "citation"
published: "2015-01-01T00:00:00"
score: 10.0
tags: ["paper", "banking-fiscal-monetary-policy", "Financial Markets and Investment Strategies", "Banking stability, regulation, efficiency", "Economic theories and models"]
---

# Musings on Liquidity

[查看原文](https://doi.org/10.1057/9781137389237_1)

## 一句话结论

> 该论文泛泛讨论市场流动性的重要性、测量和管理，但未涉及任何指定的关键词主题。

## 论文信息

- **作者**：Andria van der Merwe
- **来源**：Palgrave Macmillan US eBooks
- **发布时间**：2015-01-01
- **相关度评分**：10.0
- **DOI**：[https://doi.org/10.1057/9781137389237_1](https://doi.org/10.1057/9781137389237_1)

## 相关性评分

- **商业银行**：1.0/10（最高匹配）
- **货币政策**：1.0/10
- **财政政策**：1.0/10
- **评分依据**：论文摘要

> 最高匹配领域：商业银行 1.0/10。论文标题为'Musings on Liquidity'，摘要讨论市场流动性的一般概念，未涉及商业银行、货币政策或财政政策的具体研究问题、方法或结果。内容仅为一般性思考，与三个领域均无实质关联。

<details open>
<summary><strong>中文摘要</strong></summary>

流动性重要且不可或缺。流动性被视为有效市场的假定特征，但正如谚语中下雨时找不到的雨伞，我们往往只在流动性缺失时才关注它¹。何为市场流动性？能否衡量市场流动性？能否管理市场流动性？  
关键词：资产价格（Asset Price）；贴现因子（Discount Factor）；价格发现（Price Discovery）；期货合约（Future Contract）；做市商（Market Maker）  
（以上关键词由机器添加，并非作者提供。此过程为实验性，关键词可能随学习算法优化而更新。）

</details>

<details>
<summary><strong>英文摘要</strong></summary>

Liquidity is important and necessary. Liquidity is an assumed characteristic of a well-functioning market, but we only pay attention to it when it is absent—like the proverbial umbrella that is missing when it rains.1 What is market liquidity? Can we measure market liquidity? Can we manage market liquidity?KeywordsAsset PriceDiscount FactorPrice DiscoveryFuture ContractMarket MakerThese keywords were added by machine and not by the authors. This process is experimental and the keywords may be updated as the learning algorithm improves.

</details>

<details>
<summary><strong>发现与关联证据</strong></summary>

- **channel**：citation_expansion
- **relation**：related_to_seed
- **seed_paper_id**：https://doi.org/10.2139/ssrn.2635522
- **seed_title**：The Impact of Liquidity Regulation on Banks
- **seed_score**：96.0

</details>

---

_知识库更新时间：2026-07-14T03:57:10.407938_
