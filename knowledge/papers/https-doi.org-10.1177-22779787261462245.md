---
title: "Does UPI Adoption Reduce Demand for Physical Currency in India? An Asymmetric Analysis Using NARDL"
...
# Does UPI Adoption Reduce Demand for Physical Currency in India? An Asymmetric Analysis Using NARDL

[查看原文](https://doi.org/10.1177/22779787261462245)

## 一句话结论

> 本文利用NARDL模型研究印度UPI交易增长对流通中现金的非对称影响，发现正冲击导致现金增加（互补效应），负冲击导致现金大幅减少（预防性动机），对印度央行货币政策和现金管理有启示。
...
印度统一支付接口（UPI）已成为全球最大的实时支付系统之一，然而，流通中货币（CwP），即银行体系外持有的实物纸币与硬币，却与之同步持续增长。本文利用2016年8月至2026年3月的月度数据（共116个观测值），采用非线性自回归分布滞后（NARDL）模型，考察UPI交易增长是否对CwP产生非对称效应。该分析基于Whitesell（1989）提出的支付工具选择交易成本理论，并拓展了Alvarez和Lippi（2009）的动态现金库存模型，该模型预测，数字支付可用性的收缩所引发的预防性货币积累，其幅度将不成比例地大于正常扩张期间的边际减少。消费者价格指数（CPI）与印度储备银行政策回购利率作为控制变量纳入模型，同时引入加法虚拟变量以处理废钞令（2016年11月）与新冠疫情（2020年3月）带来的结构性断点。边界检验确认了协整关系（F=7.507），Wald检验则证实了显著的长期非对称性（F=11.629，p=0.001）。长期来看，UPI的正向冲击与CwP的上升相关联，这与支付工具互补性而非替代性一致；而负向冲击则具有更大的反向效应，反映了预防性动机。误差修正系数为-0.374，表明每月向均衡状态调整约37%。该研究结果对印度储备银行的货币政策框架及印度的实物现金管理议程具有直接启示意义。JEL分类号：E41, E52, O33, G21, E47
