...
[查看原文](http://arxiv.org/abs/2607.18601v1)
...
我们研究了用于估计溢出效应的实验的最优设计与分析。在假设已知（例如线性）暴露映射的前提下，我们刻画了能够最小化针对一大类不可观测变量分布的最坏情况渐近方差的处理分配分布与基于回归的估计量。该设计问题产生了一个直观的解决方案，其中规划者在溢出信号强度与溢出变异扩散之间进行权衡。分析问题则产生了一个简单的重新中心化工具变量估计量，以最佳地利用这种变异。该框架在若干基准情形（如聚类暴露）中提供了自然的解决方案，并为一般网络（包括二分图设定）提出了计算上易于处理的近似方法。我们基于发展经济学的两个应用在半合成实验中展示了这些新工具。我们的方法在两个实验中都大幅降低了标准误，使有效样本量增加了50%-100%或更多。
...
_知识库更新时间：2026-07-22T04:03:49.092590_
