---
title: "银行风险承担与竞争理论再探讨"
paper_id: "https://doi.org/10.1111/j.1540-6261.2005.00763.x"
source: "citation"
published: "2005-05-03T00:00:00"
score: 80.0
tags: ["paper", "banking-fiscal-monetary-policy", "Banking stability, regulation, efficiency", "Corporate Finance and Governance", "Global Financial Crisis and Policies"]
---

# 银行风险承担与竞争理论再探讨

> **英文原标题**：The Theory of Bank Risk Taking and Competition Revisited

[查看原文](https://doi.org/10.1111/j.1540-6261.2005.00763.x)

## 一句话结论

> 该论文质疑了传统观点（竞争导致银行风险增加），通过理论和实证分析表明市场集中度提高反而可能增加银行风险，并指出现有文献的脆弱性。

## 论文信息

- **作者**：John H. Boyd, Gianni De Nicolò
- **来源**：The Journal of Finance
- **发布时间**：2005-05-03
- **相关度评分**：80.0
- **DOI**：[https://doi.org/10.1111/j.1540-6261.2005.00763.x](https://doi.org/10.1111/j.1540-6261.2005.00763.x)

## 相关性评分

- **商业银行**：8.0/10（最高匹配）
- **货币政策**：3.0/10
- **财政政策**：1.0/10
- **评分依据**：论文摘要

> 最高匹配领域：商业银行 8.0/10。commercial_banking: 论文核心研究银行风险承担与竞争的关系，直接属于商业银行领域，评分8。monetary_policy: 论文提及对监管者和央行官员的影响，但研究问题本身不涉及货币政策工具或传导，仅作为背景，评分3。fiscal_policy: 论文未涉及财政政策，评分1。

<details open>
<summary><strong>中文摘要</strong></summary>

摘要 大量文献得出结论：当面临竞争加剧时，银行会理性地选择风险更高的资产组合。我们认为，这些文献对监管机构和中央银行家产生了显著影响。通过回顾实证文献，我们发现相关证据最好被描述为“喜忧参半”。随后，我们指出现有关于该主题的理论分析较为薄弱，因为存在一些基本的风险激励机制，其作用方向恰恰相反——即随着市场集中度提高，银行的风险反而会上升。这些机制应成为银行竞争模型的核心要素。

</details>

<details>
<summary><strong>英文摘要</strong></summary>

ABSTRACT There is a large body of literature that concludes that—when confronted with increased competition—banks rationally choose more risky portfolios. We argue that this literature has had a significant influence on regulators and central bankers. We review the empirical literature and conclude that the evidence is best described as “mixed.” We then show that existing theoretical analyses of this topic are fragile, since there exist fundamental risk‐incentive mechanisms that operate in exactly the opposite direction, causing banks to become more risky as their markets become more concentrated. These mechanisms should be essential ingredients of models of bank competition.

</details>

## 深度解读

> 分析依据：**摘要分析**

### 核心结论

本文回顾了关于银行竞争加剧导致风险承担增加的文献，指出该结论对监管者和央行有重要影响。然而，实证证据“混合”，且现有理论分析脆弱，因为存在相反的风险激励机制：市场集中度提高时，银行可能变得更冒险。作者认为这些机制应成为银行竞争模型的核心要素。

### 主要创新

- 指出银行竞争与风险承担关系的实证证据为“混合”，挑战传统结论。
- 揭示现有理论分析的脆弱性，发现存在反向风险激励机制。
- 提出市场集中度提高时银行风险增加的新机制，应纳入模型。

### 研究方法

摘要未提供具体研究方法，仅提及对实证文献的回顾和理论分析。

### 关键结果

银行竞争加剧导致风险承担增加的文献结论对监管和央行政策有显著影响。；实证证据被描述为“混合”，不支持单一方向。；存在基本风险激励机制，使银行在市场更集中时变得更冒险。

### 技术栈

- 摘要未提供具体算法、工具或数学方法。

### 方法优势

- 挑战主流观点，提出反向机制，具有理论创新性。
- 强调实证证据的混合性，促进更谨慎的政策应用。
- 为银行竞争模型提供新的理论方向。

### 主要局限

- 论文局限：摘要未提供实证分析细节或模型推导。当前证据局限：仅基于摘要，无法评估方法严谨性和数据可靠性。

### 与当前研究方向的关联

高度相关：直接涉及银行风险承担、竞争、金融稳定和监管政策，与银行信贷、资本监管、宏观审慎政策等关键词紧密关联。

<details>
<summary><strong>发现与关联证据</strong></summary>

- **channel**：citation_expansion
- **relation**：referenced_by_seed
- **seed_paper_id**：https://doi.org/10.1007/s11115-026-01032-1
- **seed_title**：Brief Exploration on the Evolution and Dynamics of Bank Stability Research
- **seed_score**：106.0

</details>

---

_知识库更新时间：2026-07-13T03:50:29.022838_
