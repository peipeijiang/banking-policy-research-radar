---
title: "外国银行：趋势与影响"
paper_id: "https://doi.org/10.1111/jmcb.12092"
source: "citation"
published: "2014-01-27T00:00:00"
score: 80.0
tags: ["paper", "banking-fiscal-monetary-policy", "Banking stability, regulation, efficiency", "Islamic Finance and Banking Studies", "Global Financial Crisis and Policies"]
---

# 外国银行：趋势与影响

> **英文原标题**：Foreign Banks: Trends and Impact

[查看原文](https://doi.org/10.1111/jmcb.12092)

## 一句话结论

> 该论文通过构建外资银行所有权数据库，分析了外资银行对国内信贷的影响，发现其负面效应仅存在于低收入国家、市场占有率低、合同执行成本高、信用信息有限以及母国距离远的国家。

## 论文信息

- **作者**：Stijn Claessens, Neeltje van Horen
- **来源**：Journal of money credit and banking
- **发布时间**：2014-01-27
- **相关度评分**：80.0
- **DOI**：[https://doi.org/10.1111/jmcb.12092](https://doi.org/10.1111/jmcb.12092)

## 相关性评分

- **商业银行**：8.0/10（最高匹配）
- **货币政策**：2.0/10
- **财政政策**：2.0/10
- **评分依据**：论文摘要

> 最高匹配领域：商业银行 8.0/10。commercial_banking: 论文研究外国银行在各国金融中介中的作用，分析其对信贷供给的影响，并考虑银行特征、制度环境等，直接属于商业银行领域，核心贡献。monetary_policy: 仅间接提及信贷，未涉及货币政策工具、传导或政策含义。fiscal_policy: 未涉及政府收入、支出、债务等财政政策内容。

<details open>
<summary><strong>中文摘要</strong></summary>

过去二十年间，外资银行在国内金融中介活动中的重要性显著提升，这进一步凸显了理解其行为特征的必要性。我们构建并公开了一个全新的综合性数据库，涵盖1995-2009年间137个国家5,324家银行的股权结构信息（包括外资银行的母国归属）。研究显示，多国外资银行参与度大幅提升，但在东道国与母国特征、双边投资模式及银行个体属性方面存在显著异质性。就影响层面而言，我们证实私人信贷与外资银行参与度之间的关系主要取决于东道国及银行自身特征。具体而言，外资银行仅在低收入国家、市场份额有限的国家、合同执行成本高昂且信用信息获取受限的国家，以及母国距离较远的国家，才会对信贷产生负面影响。这表明，将异质性因素（包括双边所有权结构）纳入考量，对于更深入理解外资银行所有权的影响至关重要。

</details>

<details>
<summary><strong>英文摘要</strong></summary>

Over the past two decades, foreign banks have become much more important in domestic financial intermediation, heightening the need to understand their behavior. We introduce a new, comprehensive database, made publicly available, on bank ownership (including the home country of foreign banks) for 5,324 banks in 137 countries over the period 1995–2009. We document large increases in foreign bank presence in many countries, but with substantial heterogeneity in terms of host and banks’ home countries, bilateral investment patterns, and bank characteristics. In terms of impact, we document that the relation between private credit and foreign bank presence importantly depends on host country and banks’ characteristics. Specifically, foreign banks only seem to have a negative impact on credit in low‐income countries, in countries where they have a limited market share, where enforcing contracts is costly and where credit information is limited available, and when they come from distant home countries. This shows that accounting for heterogeneity, including bilateral ownership, is crucial to better understand the implications of foreign bank ownership.

</details>

## 深度解读

> 分析依据：**摘要分析**

### 核心结论

本文研究了过去二十年外国银行在国内金融中介中日益重要的角色及其影响。作者构建了一个涵盖137个国家5324家银行（1995-2009年）的银行所有权数据库，并公开提供。研究发现，许多国家外国银行存在显著增加，但在东道国、母国、双边投资模式和银行特征方面存在异质性。关于影响，私人信贷与外国银行存在之间的关系取决于东道国和银行特征：外国银行仅在低收入国家、市场份额有限、合同执行成本高、信用信息有限以及母国距离遥远的国家中对信贷产生负面影响。这表明考虑异质性（包括双边所有权）对于理解外国银行所有权的影响至关重要。

### 主要创新

- 构建并公开了一个涵盖137个国家、5324家银行、时间跨度1995-2009年的银行所有权综合数据库。
- 系统记录了外国银行存在的增长趋势及其在国家和银行层面的异质性。
- 揭示了外国银行对私人信贷的影响取决于东道国收入水平、市场结构、制度环境及母国距离等条件。
- 强调了双边所有权信息在分析外国银行影响中的重要性。

### 研究方法

论文采用描述性统计和计量回归分析，利用新构建的银行所有权面板数据，考察外国银行存在与私人信贷的关系，并通过交互项分析异质性效应。

### 关键结果

外国银行在许多国家的存在显著增加，但存在国家、母国和银行层面的异质性。；外国银行对私人信贷的负面影响仅出现在低收入国家、市场份额有限、合同执行成本高、信用信息有限以及母国距离遥远的国家。；考虑异质性（包括双边所有权）对于理解外国银行影响至关重要。

### 技术栈

- 摘要未提供具体算法或工具，但可能涉及面板数据回归、固定效应模型等计量方法。

### 方法优势

- 构建了大规模、跨国、跨时期的银行所有权数据库，数据公开透明。
- 系统分析了外国银行存在的趋势和异质性，填补了文献空白。
- 通过条件分析揭示了外国银行影响的异质性，提供了更细致的政策启示。

### 主要局限

- **论文本身局限**：摘要未明确讨论局限性，但可能包括数据时间范围（1995-2009年）的时效性、未考虑金融危机等结构性变化。
- **current evidence limitations**：当前仅提供摘要，无法评估具体计量方法、稳健性检验、内生性处理等细节。

### 与当前研究方向的关联

论文直接涉及银行信贷、金融中介、金融稳定等关键词，通过分析外国银行对私人信贷的影响，与银行信贷、金融中介、宏观审慎政策等主题高度相关。

<details>
<summary><strong>发现与关联证据</strong></summary>

- **channel**：citation_expansion
- **relation**：referenced_by_seed
- **seed_paper_id**：https://doi.org/10.3390/jrfm19070510
- **seed_title**：Does Liquidity Risk Impact Asset Quality and Financial Stability? Evidence from Uzbekistan Commercial Banks
- **seed_score**：88.0

</details>

---

_知识库更新时间：2026-07-13T04:30:56.251971_
