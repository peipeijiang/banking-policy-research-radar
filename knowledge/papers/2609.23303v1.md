...
[查看原文](http://arxiv.org/abs/2609.23303v1)

## 一句话结论

> 论文提出自适应L1惩罚上确界统计量，用于条件分位数约束下参数的统一推断，并给出预估计干扰参数下的渐近理论与自助法验证。
...
许多结构和动态经济模型都意味着关键参数通过条件分位数限制条件得到识别。基于Bierens（1990）的指数加权方法以及条件矩限制条件下惩罚最大统计量的近期进展（Chen等，2025），我们为这类参数开发了一个统一的推断框架。我们提出了一种自适应$\ell_1$惩罚上确界统计量，该统计量将条件限制条件转化为一个连续的无条件矩条件集合，并跨分位数指标聚合证据。惩罚项对权重方向上的最大化进行正则化。在所述一致性条件下，已知参数的自适应选择器在局部最大最小功效上不低于未惩罚检验，并且只要某个正的候选惩罚具有严格大于零惩罚的总体最大最小准则，就会产生严格的局部最大最小功效增益。我们将该理论扩展到具有预估计冗余参数的设定，刻画了极限过程中由代入步骤引起的附加项。我们推导了一个解析校正的方差估计量，以考虑代入估计的不确定性，并在原假设和局部备择序列下建立了高斯乘子自助法的有效性。蒙特卡洛模拟表明，所提出的CvM-KS聚合方案在线性设计中的预估计下，拒绝率相对接近名义水平，而在非线性设计中随着样本量增大而趋近名义水平。所报告的自适应方法与未惩罚方法之间的功效比较因设计而异，自适应方法在某些方向上具有更高的经验拒绝频率。
...
_知识库更新时间：2026-09-22T05:24:40.025068_
