...
[查看原文](http://arxiv.org/abs/2608.09027v1)

## 一句话结论

> 论文提出了一种新的渐近框架，用于在部分识别和抽样噪声下推导近似最优的处理分配，并应用于污染结果、消费者剩余和实验聚合等问题。
...
我们提出了一种新的渐近框架，用于在数据采样噪声与部分识别所导致的基本不确定性叠加时，推导近似最优的处理分配方案。我们将简化型参数围绕其“最不利”配置进行重新中心化，并考虑同时导致采样不确定性和部分识别程度递减的漂移参数序列。我们将极限决策问题刻画为一个具有适当极限识别集的正态位置平移模型。我们将所提出的方法应用于以下问题：存在污染结果时的处理选择问题、部分识别消费者剩余下的稳健福利分析，以及为政策采纳而聚合实验估计值的问题。
...
_知识库更新时间：2026-08-11T02:46:38.330371_
