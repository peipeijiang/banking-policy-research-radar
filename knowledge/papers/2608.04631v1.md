...
[查看原文](http://arxiv.org/abs/2608.04631v1)

## 一句话结论

> 提出一种贝叶斯分层局部投影框架，用于在短时间序列中估计脉冲响应，并通过美国价格数据展示其对不同价格指数异质性反应的估计效果。
...
在短期及极短期样本中估计经济冲击的动态效应，因自由度不足而受到限制。我们提出一种基于贝叶斯分层框架的解决方案，用于在相关时间序列面板中估计局部投影（LP）脉冲响应函数。该框架明确允许非平衡面板的存在，其中部分序列明显短于其他序列，从而使短序列能够在自身数据极少或缺失的预测时域上，从长序列中借用信息。由于各序列可能表现出异质性动态特征，我们开发了一种稀疏有限混合池，根据单位脉冲响应轮廓的相似性对其进行聚类。模拟结果表明，在时间序列较短的情况下，我们的方法相较于标准方法能显著提高LP估计精度，同时对较长的时间序列也能产生相似的LP结果。利用美国价格数据集并辅以调查问卷数据，我们发现供应链冲击和石油冲击会引发不同价格指标的异质性反应，其中总体价格指数的反应比核心价格指数更为剧烈，商品价格的变化幅度也大于服务价格。
...
_知识库更新时间：2026-08-06T03:57:42.787856_
