---
title: "The Provision of Liquidity by High‐Frequency Participants"
paper_id: "https://doi.org/10.1111/fire.12040"
source: "citation"
published: "2014-04-07T00:00:00"
score: 0.0
tags: ["paper", "banking-fiscal-monetary-policy", "Financial Markets and Investment Strategies", "Corporate Finance and Governance", "Housing Market and Economics"]
---

# The Provision of Liquidity by High‐Frequency Participants

[查看原文](https://doi.org/10.1111/fire.12040)

## 一句话结论

> 该论文研究高频交易者如何通过多价位订单策略持续提供流动性，并发现其行为有助于缓解临时流动性失衡，改善市场流动性，但对市场深度和非HFT交易者存在潜在影响。

## 论文信息

- **作者**：Elvis Jarnecic, Mark Snape
- **来源**：Financial Review
- **发布时间**：2014-04-07
- **相关度评分**：0.0
- **DOI**：[https://doi.org/10.1111/fire.12040](https://doi.org/10.1111/fire.12040)

## 相关性评分

- **商业银行**：0.0/10（最高匹配）
- **货币政策**：0.0/10
- **财政政策**：0.0/10
- **评分依据**：论文摘要

> 最高匹配领域：商业银行 0.0/10。论文研究高频交易者（HFT）在限价订单簿中的流动性供给行为，属于市场微观结构领域，与商业银行、货币政策、财政政策均无实质关联。未涉及银行信贷、金融中介、银行监管、中央银行政策工具或政府财政工具。

<details open>
<summary><strong>中文摘要</strong></summary>

本文研究了限价订单簿中高频参与者与其他参与者的订单提交策略及流动性供给情况。结果表明，高频参与者在限价订单簿中以多个价格提交订单，并集中于报价内或报价附近。这一行为转化为持续性的流动性供给，且无论在快速、慢速还是波动剧烈的市场中均表现稳健，共同表明高频参与者解决了限价订单簿中的临时流动性失衡。该证据与高频交易（HFT）改善市场流动性的观点一致，但围绕高频参与者对市场深度的影响以及非高频交易（non-HFT）参与者交易困难的问题仍然存在。

</details>

<details>
<summary><strong>英文摘要</strong></summary>

Abstract This paper examines the order submission strategies and supply of liquidity by high‐frequency participants versus the remainder of participants in the limit order book. The results show that high‐frequency participants submit orders at multiple prices in the limit order book, concentrated at or within the quote. This activity translates into the provision of liquidity on an on‐going basis, which is robust to fast versus slow and volatile markets, together suggesting that high‐frequency participants resolve temporal liquidity imbalances in the limit order book. The evidence is consistent with high‐frequency trading (HFT) improving market liquidity, but there remain issues surrounding high‐frequency participants’ effect on market depth and the difficulty of trading of non‐HFT participants.

</details>

<details>
<summary><strong>发现与关联证据</strong></summary>

- **channel**：citation_expansion
- **relation**：related_to_seed
- **seed_paper_id**：https://doi.org/10.2139/ssrn.2635522
- **seed_title**：The Impact of Liquidity Regulation on Banks
- **seed_score**：96.0

</details>

---

_知识库更新时间：2026-07-14T03:57:10.407437_
