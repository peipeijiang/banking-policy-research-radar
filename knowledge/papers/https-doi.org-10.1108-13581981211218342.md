---
title: "流动性风险与银行体系绩效"
paper_id: "https://doi.org/10.1108/13581981211218342"
source: "citation"
published: "2012-05-04T00:00:00"
score: 56.0
tags: ["paper", "banking-fiscal-monetary-policy", "Banking stability, regulation, efficiency", "Islamic Finance and Banking Studies", "Corporate Finance and Governance"]
---

# 流动性风险与银行体系绩效

> **英文原标题**：Liquidity risk and performance of banking system

[查看原文](https://doi.org/10.1108/13581981211218342)

## 一句话结论

> 本文研究巴基斯坦银行流动性风险对盈利能力的影响，发现流动性缺口和不良贷款显著加剧流动性风险并降低盈利能力。

## 论文信息

- **作者**：Ahmed Arif, Ahmed Nauman Anees
- **来源**：Journal of Financial Regulation and Compliance
- **发布时间**：2012-05-04
- **相关度评分**：56.0
- **DOI**：[https://doi.org/10.1108/13581981211218342](https://doi.org/10.1108/13581981211218342)

<details open>
<summary><strong>中文摘要</strong></summary>

目的 本文旨在研究巴基斯坦银行的流动性风险，并评估其对银行盈利能力的影响。设计/方法/路径 数据来源于2004年至2009年间22家巴基斯坦银行的资产负债表、利润表及附注。采用多元回归分析评估流动性风险对银行盈利能力的影响。研究发现 多元回归结果显示，流动性风险显著影响银行盈利能力，其中流动性缺口与不良贷款是加剧流动性风险的两大因素，二者与盈利能力呈负相关关系。研究局限/启示 受数据可得性限制，本文研究时段为2004年至2009年。但由于样本涵盖22家银行，构成巴基斯坦银行体系的主体部分，因此样本期间并未削弱研究结论的有效性。此外，本文仅以盈利能力作为绩效衡量指标，未涉及导致流动性风险的经济因素。原创性/价值 本文是首篇针对巴基斯坦银行体系流动性风险的研究。过往研究者及实务工作者对流动性风险关注不足。本文有助于理解流动性风险的构成要素及其对银行体系盈利能力的影响。作者建议当代风险管理者通过持有充足现金资源来缓解流动性风险，这将缩小流动性缺口，进而降低对回购市场的依赖。

</details>

<details>
<summary><strong>英文摘要</strong></summary>

Purpose The purpose of this paper is to examine liquidity risk in Pakistani banks and evaluate the effect on banks' profitability. Design/methodology/approach Data are retrieved from the balance sheets, income statements and notes of 22 Pakistani banks during 2004‐2009. Multiple regressions are applied to assess the impact of liquidity risk on banks' profitability. Findings The results of multiple regressions show that liquidity risk affects bank profitability significantly, with liquidity gap and non‐performing as the two factors exacerbating the liquidity risk. They have a negative relationship with profitability. Research limitations/implications The period studied in this paper is 2004‐2009, due to availability of the data. However, the sample period does not impair the findings since the sample includes 22 banks, which constitute the main part of the Pakistani banking system. Moreover, only profitability is used as the measure of performance. Economic factors contributing to liquidity risk are not covered in this paper. Originality/value This is the first paper addressing the liquidity risk faced by the Pakistani banking system. Past researchers and practitioners have not given the proper attention to liquidity risk. This paper helps in understanding the factors of liquidity risk and their impact on the profitability of the banking system. The authors emphasise contemporary risk managers to mitigate liquidity risk by having sufficient cash resources. This will reduce the liquidity gap, thereby reducing the dependence on repo market.

</details>

## 深度解读

> 分析依据：**摘要分析**

### 核心结论

本文旨在研究巴基斯坦银行的流动性风险及其对银行盈利能力的影响。数据来源于2004-2009年间22家巴基斯坦银行的资产负债表、利润表和附注。采用多元回归分析评估流动性风险对银行盈利能力的影响。结果表明，流动性风险显著影响银行盈利能力，其中流动性缺口和不良贷款是加剧流动性风险的两个因素，它们与盈利能力呈负相关。研究期间为2004-2009年，受数据可得性限制，但样本涵盖巴基斯坦银行体系的主要部分。仅使用盈利能力作为绩效衡量指标，未涵盖导致流动性风险的经济因素。本文首次关注巴基斯坦银行体系面临的流动性风险，有助于理解流动性风险因素及其对银行体系盈利能力的影响，并强调风险管理者应通过持有充足现金资源来缓解流动性风险，缩小流动性缺口，减少对回购市场的依赖。

### 主要创新

- 首次针对巴基斯坦银行体系研究流动性风险及其对盈利能力的影响。
- 识别出流动性缺口和不良贷款是加剧流动性风险的关键因素。
- 强调流动性风险在以往研究和实践中未得到足够重视。
- 为风险管理者提供缓解流动性风险的具体建议，如持有充足现金资源。

### 研究方法

从22家巴基斯坦银行2004-2009年的资产负债表、利润表和附注中提取数据，应用多元回归分析评估流动性风险对银行盈利能力的影响。

### 关键结果

流动性风险显著影响银行盈利能力。；流动性缺口和不良贷款是加剧流动性风险的两个因素。；流动性缺口和不良贷款与盈利能力呈负相关。

### 技术栈

- 多元回归分析

### 方法优势

- 首次针对巴基斯坦银行体系进行流动性风险研究，填补了该领域空白。
- 使用实际银行数据，样本覆盖巴基斯坦银行体系的主要部分。
- 研究结果具有明确的政策含义，为风险管理提供指导。

### 主要局限

- 论文局限：研究期间仅为2004-2009年，受数据可得性限制；仅使用盈利能力作为绩效指标，未涵盖其他绩效维度；未分析导致流动性风险的经济因素。
- 当前证据局限：仅基于摘要，无法确认具体回归模型设定、变量定义、样本银行名单、稳健性检验细节等。

### 与当前研究方向的关联

本文直接关注流动性风险与银行盈利能力，与银行信贷与风险承担、资本和流动性监管、金融稳定等关键词高度相关。研究结果对理解流动性风险的影响及风险管理具有参考价值。

<details>
<summary><strong>发现与关联证据</strong></summary>

- **channel**：citation_expansion
- **relation**：related_to_seed
- **seed_paper_id**：https://doi.org/10.2139/ssrn.1951799
- **seed_title**：A Pigovian Approach to Liquidity Regulation
- **seed_score**：111.0

</details>

---

_知识库更新时间：2026-07-14T03:57:10.400811_
