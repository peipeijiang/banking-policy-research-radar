---
title: "外生流动性冲击对银行融资成本的影响：来自无担保银行间市场的微观证据"
paper_id: "https://doi.org/10.71609/iheid-4v79-af73"
source: "citation"
published: "2024-09-26T00:00:00"
score: 106.0
tags: ["paper", "banking-fiscal-monetary-policy", "Banking stability, regulation, efficiency", "Housing Market and Economics", "Global Financial Crisis and Policies"]
---

# 外生流动性冲击对银行融资成本的影响：来自无担保银行间市场的微观证据

> **英文原标题**：The impact of exogenous liquidity shocks on banks funding costs: microevidence from the unsecured interbank market

[查看原文](http://hdl.handle.net/10419/201697)

## 一句话结论

> 本文研究外生流动性冲击（存款流出和量化宽松缩减）对银行间市场融资成本的影响，发现资本充足和流动性强的银行支付更低成本，且借贷关系和央行流动性有助于缓解冲击。

## 论文信息

- **作者**：Miguel Sarmiento
- **来源**：Econstor (Econstor)
- **发布时间**：2024-09-26
- **相关度评分**：106.0
- **DOI**：[https://doi.org/10.71609/iheid-4v79-af73](https://doi.org/10.71609/iheid-4v79-af73)

<details open>
<summary><strong>中文摘要</strong></summary>

本文研究了无担保银行间市场中 exogenous liquidity shocks（外生流动性冲击）的影响。我们评估了由银行层面存款外流引发的 idiosyncratic liquidity shocks（特质性流动性冲击）以及2013年5月至9月期间美国 taper（缩减购债）相关的 aggregate liquidity shock（总体流动性冲击）的效果。研究发现，两种流动性冲击均与银行间贷款价格上升相关，但溢价幅度及对银行间流动性获取的影响因借款方特征而异。资本充足率更高、流动性更强的银行不仅倾向于支付更低的流动性成本（这与市场纪律的证据一致），而且能更好地吸收外生流动性冲击的影响，这表明资本充足率和流动性比率具有益处。我们的结果表明，在特质性流动性冲击期间，贷款关系有助于缓解融资成本，而中央银行流动性则有助于平抑总体流动性冲击的影响。研究结果对金融稳定和货币政策传导均具有启示意义。

</details>

<details>
<summary><strong>英文摘要</strong></summary>

This paper examines the impact of exogenous liquidity shocks in the unsecured interbank market. We evaluate the effects of idiosyncratic liquidity shocks - arising from deposits outflow at the bank level - and of the aggregate liquidity shock related to the U.S. tapering observed between May and September of 2013. We find that both liquidity shocks are associated with higher interbank loan prices, albeit the magnitude of the overprice and the impact on the access to interbank liquidity differ depending on the borrower-specific characteristics. More capitalized and liquid banks tend to pay less for liquidity - concurrent with evidence on market discipline - but also can absorb better the impact of exogenous liquidity shocks, suggesting benefits from capital and liquidity ratios. Our results suggest that lending relationships can alleviate funding costs during idiosyncratic liquidity shocks, while central bank liquidity contributes to smooth the impact of aggregate liquidity shocks. Results have implications for both financial stability and monetary policy transmission.

</details>

## 深度解读

> 分析依据：**全文深读**

### 核心结论

本文研究了无担保银行间市场中，外生流动性冲击对银行融资成本的影响。作者分别评估了银行层面存款外流导致的特质性流动性冲击，以及2013年5月至9月美国量化宽松缩减引发的总体流动性冲击。研究发现，两种流动性冲击均与银行间贷款价格上升相关，但溢价幅度和流动性获取能力因借款银行特征而异。资本充足且流动性强的银行支付更低的流动性成本，并能更好地吸收外生冲击，表明资本和流动性比率具有益处。此外，贷款关系有助于缓解特质性冲击下的融资成本，而央行流动性则有助于平抑总体冲击的影响。结果对金融稳定和货币政策传导具有启示。

### 主要创新

- 同时考察特质性和总体流动性冲击对银行间市场的影响
- 利用微观数据揭示银行特征（资本、流动性）在冲击传导中的异质性作用
- 区分贷款关系和央行流动性在缓解不同冲击中的作用
- 为资本和流动性监管的益处提供微观证据

### 研究方法

论文采用实证分析方法，利用无担保银行间市场的微观数据，通过回归模型估计外生流动性冲击对银行间贷款价格的影响，并控制银行特征和固定效应。

### 关键结果

特质性和总体流动性冲击均导致银行间贷款价格上升；资本充足且流动性强的银行支付更低的流动性成本；贷款关系有助于缓解特质性冲击下的融资成本；央行流动性有助于平抑总体流动性冲击的影响

### 技术栈

- 输入内容未提供

### 方法优势

- 使用微观数据提供银行层面的证据
- 同时分析两种不同类型的流动性冲击
- 研究结果对金融稳定和货币政策具有政策含义

### 主要局限

- 输入内容未提供

### 与当前研究方向的关联

论文直接研究银行间市场、流动性冲击、市场纪律、货币政策传导和金融稳定，与关键词高度相关。

<details>
<summary><strong>发现与关联证据</strong></summary>

- **channel**：citation_expansion
- **relation**：related_to_seed
- **seed_paper_id**：https://doi.org/10.1093/rof/rfv058
- **seed_title**：The Impact of Liquidity Regulation on Bank Intermediation
- **seed_score**：95.0

</details>

---

_知识库更新时间：2026-07-13T06:39:35.604275_
